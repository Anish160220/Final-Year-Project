          ...
          <div class="widget-content nopadding"> {{-- The enctype attribute specifies how the form-data should be encoded when submitting it to the server.The enctype attribute can be used only if method="post".No characters are encoded. This value is required when you are using forms that have a file upload control --}}
            <form enctype="multipart/form-data" class="form-horizontal" method="post" action="{{ url('/admin/edit-product/'.$productDetails->id)}}" name="edit_product" id="edit_product" novalidate="novalidate">{{ csrf_field()}}
              ...
              <div class="control-group">
                ...
                <div class="controls">
                 <textarea name="description" id="description">{{ $productDetails->product_color }} </textarea>  
                </div>
              </div>
              <div class="control-group">
              ...
              <div class="control-group">
                ...
                <div class="controls">
                  ...
                  @if(!empty($productDetails->image))
                    <img style="width:40px" src="{{ asset('/images/backend_images/products/small/'.$productDetails->image) }}"> | <a href="{{ url('/admin/delete-product-image/'.$productDetails->id) }}" class="btn btn-danger btn-mini">Delete</a>
                  @endif
                </div>
              </div>
              <div class="control-group">
